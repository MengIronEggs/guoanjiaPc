...
import { objFn } from "~/plugins/axios.js";
    export default {
        ...
        computed: {
            subway() {
                return this.$store.state.subway
            }
        },
        created(){
        ...
        methods: {
            ...
            getAreaDistribution(){
                ...
                objFn.Axios(
                    "agenthouseCutomer/pc/HouseInfoController/getAreaDistribution",
                    "post",
                    data,
                    { interfaceType: "NEW_HOUSE" }
                )
                // this.post("pc/HouseInfoController/getAreaDistribution", data)
                .then(res => {
                    if (res.code !== 0) {
                    ...
                    if(item.totalNum!=0){
                        ...
                        label.addEventListener("click", () => {
                            ...
                            if(item.districtName!=="海淀" && item.districtName!=="昌平" && item.districtName!=="顺义"){
                                ...
                                objFn.Axios(
                                    "agenthouseCutomer/pc/HouseInfoController/getMapHouseList",
                                    "post",
                                    data,
                                    { interfaceType: "NEW_HOUSE" }
                                )
                                // this.post("pc/HouseInfoController/getMapHouseList",data)
                                .then(res=>{
            ...
            createSubwayCover(subwayList,subwayType){
                ...
                // this.post("pc/HouseInfoController/getSubwayLineHouseCount",data)//地图里展示站点套数
                .then(res=>{
            ...
            getHouseDistribution(cb = ()=>{}){
                ...
                objFn.Axios(
                    "agenthouseCutomer/pc/HouseInfoController/getCommunityByScreenCoordinate",
                    "post",
                    data,
                    { interfaceType: "NEW_HOUSE" }
                )
                // this.post("pc/HouseInfoController/getCommunityByScreenCoordinate", data)
                .then(res => {
                    if (res.code !== 0) {
                        this.showErrorTip(res.msg);
                    } else {
            ...
            scopeSubway(data){
                ...
                objFn.Axios(
                    "agenthouseCutomer/pc/HouseInfoController/getCommunityNearbyPoint",
                    "post",
                    Axiosdata,
                    { interfaceType: "NEW_HOUSE" }
                )
                // this.post("pc/HouseInfoController/getCommunityNearbyPoint",data)
                .then(res=>{
                    this.points = [];
